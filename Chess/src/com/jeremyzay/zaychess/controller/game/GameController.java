...
//        System.out.println(engineJar.toAbsolutePath().toString());
	...
	private void select(Position p) {
	    ...
	    Piece piece = gameState.getPieceAt(p);
        if (gameState.isInCheck()) ChessFrame.getStatusPanel().setStatus("Turn: " + gameState.getTurn(), Color.MAGENTA);
        else ChessFrame.getStatusPanel().setStatus("Turn: " + gameState.getTurn());
